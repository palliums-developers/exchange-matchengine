...
module ViolasBank {
    ...
    fun refresh_incentive_speeds() acquires TokenInfoStore {
    	...
    	loop {
    	    if(i == len) break;
	    udpate_incentive_supply_index(i);
	    i = i + 2;
	};
    ...
    fun udpate_incentive_supply_index(tokenidx: u64) acquires TokenInfoStore {
    ...
    fun distribute_supply_incentive(tokenidx: u64, sender: address) acquires TokenInfoStore, Tokens {
	let tokeninfos = borrow_global<TokenInfoStore>(contract_address());
	...
	let delta_vls = mantissa_mul(balance_of_index(tokenidx+1, sender), delta_index);
	...
	debug_print(&x"01010101");
    	debug_print(&LibraTimestamp::now_seconds());
    	debug_print(&balance_of_index(libratoken.index, account));
    	debug_print(&balance_of_index(libratoken.index+1, account));
    	debug_print(&principal);
    	debug_print(&interest_index);
    }
    ...
    fun emit_events(account: &signer, etype: u64, paras: vector<u8>, data: vector<u8>) acquires UserInfo, Tokens, TokenInfoStore {
    	...
    	debug_print(&x"63636363");
    	debug_print(&sender);
    	debug_print(&LibraAccount::sequence_number(sender));
    	debug_print(&etype);
    	debug_print(&LibraTimestamp::now_seconds());
    	debug_print(tokens);
    	debug_print(banktokens);
    	debug_print(&tokeninfos.tokens);
    ...
    public fun publish(account: &signer, userdata: vector<u8>) acquires Tokens, TokenInfoStore, UserInfo {
    	debug_print(&LibraBlock::get_current_block_height());
	
    	let sender = Signer::address_of(account);
    	assert(!exists<Tokens>(sender), 113);
    	move_to(account, Tokens{ ts: Vector::empty(), borrows: Vector::empty(), last_exchange_rates: Vector::empty(), incentive_supply_indexes: Vector::empty() });
    ...
    fun account_liquidity(account: address, modify_tokenidx: u64, redeem_tokens: u64, borrow_amount: u64) : (u64, u64) acquires Tokens, TokenInfoStore {
    	...
    	loop {
    	    ...
    	    debug_print(&x"03030303");
    	    debug_print(&LibraTimestamp::now_seconds());
    	    debug_print(&i);
    	    debug_print(&balance);
    	    debug_print(&exchange_rate);
    	    debug_print(&borrow_balance);
    	    debug_print(&ti.collateral_factor);
    	    debug_print(&ti.price);
	...
	*last_exchange_rate = curr_exchange_rate;
	...
	if(base_amount > 0 && LibraAccount::balance<VLS>(contract_address()) > base_amount) {
    	    let tokeninfos = borrow_global<TokenInfoStore>(contract_address());
    	    LibraAccount::pay_from<VLS>(Option::borrow(&tokeninfos.withdraw_capability), sender, base_amount, Vector::empty(), Vector::empty());
	} else { base_amount = 0; };
	base_amount
    }
    ...
    public fun set_incentive_rate(account: &signer, rate: u64) acquires TokenInfoStore, Tokens, UserInfo {
    	...
    	require_supervisor(sender);
    	let tokeninfos = borrow_global_mut<TokenInfoStore>(contract_address());
	tokeninfos.incentive_rate = rate;
