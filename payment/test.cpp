...
struct TestBase
{
  TestBase(){ std::cout << "base()\n"; }
  virtual ~TestBase(){ std::cout << "~base()\n"; }
};
...
int main()
{
  ...
  char query[1024];
  std::vector<std::string> keys = {"id", "project_id", "user_id", "booking_timestamp", "booking_amount", "payment_txid", "payment_timestamp", "investment_return_addr", "accumulated_gain", "status"};
  ...
  snprintf(query, sizeof(query), "SELECT * FROM bbb WHERE id=0 FOR UPDATE");
  do_select(query, keys, &vv);

  double balance = atof(vv[0]["booking_amount"].c_str());
  printf("balance: %f\n", balance);
  
  snprintf(query, sizeof(query), "UPDATE bbb SET booking_amount=booking_amount-0.01 WHERE id=0");
  if(!do_query(query)) goto FAIL;

  snprintf(query, sizeof(query), "UPDATE bbb SET booking_amount=booking_amount+0.01 WHERE id=1");
  if(!do_query(query)) goto FAIL;
...
FAIL:
  LOG(WARNING, "transaction fail, will rollback");
